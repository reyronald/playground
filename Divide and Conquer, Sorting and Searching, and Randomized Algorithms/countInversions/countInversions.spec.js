...
const testCases = [
  { input: [1,3,5,2,4,6], expected: 3 },
  { input: [1,5,3,2,4], expected: 4 },
  { input: [5,4,3,2,1], expected: 10 },
  { input: [1,6,3,2,4,5], expected: 5 },
  { input: [1,3,5,2,4,6], expected: 3 },
  { input: [9, 12, 3, 1, 6, 8, 2, 5, 14, 13, 11, 7, 10, 4, 0], expected: 56 },
...
describe('', () => {
  testCases.forEach((testCase, key) => {
    it(`Case ${+key+1}`, () => {
      // assert.equal( sortAndcount(testCase.input).count, testCase.expected );
      assert.equal( countInversions(testCase.input), testCase.expected );
    });
  });
});
